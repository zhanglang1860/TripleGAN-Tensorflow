...
########################################################################################
  ...
  def load_model(self,which_check_point=None):
    ...
    if ckpt and ckpt.model_checkpoint_path:
      ...
      print("Successfully load model from save path: %s and epoch: %s for best validation results"
  ...
  def generator(self, z, y, phase_train=True, reuse=False, split_dimension_core = 6, tt_rank = 6):
    ...
    with tf.variable_scope("Generator", reuse=reuse):
      ...
      g_1 = conv3d_transpose_tensor(x, 8, 3, all_output_shape, strides=[1, 1, 1, 1, 1], padding='SAME',kernel_name='kernel_1', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
      ...
      g_2 = conv3d_transpose_tensor(g_1, 16, 3, all_output_shape, strides=strides, padding='SAME',kernel_name='kernel_2', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
      g_2 = tf.contrib.layers.batch_norm(g_2, is_training=phase_train)
      g_2 = tf.nn.relu(g_2)
      g_2 = conv_concat(g_2, y)

      all_output_shape = (batch_size, 14, 12, 12, 16)
      g_3 = conv3d_transpose_tensor(g_2, 16, 3, all_output_shape, strides=strides, padding='SAME',kernel_name='kernel_3', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
      g_3 = tf.contrib.layers.batch_norm(g_3, is_training=phase_train)
      g_3 = tf.nn.relu(g_3)
      g_3 = conv_concat(g_3, y)

      all_output_shape = (batch_size, 28, 23, 23, 16)
      g_4 = conv3d_transpose_tensor(g_3, 16, 3, all_output_shape, strides=strides, padding='SAME',kernel_name='kernel_4', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
      g_4 = tf.contrib.layers.batch_norm(g_4, is_training=phase_train)
      g_4 = tf.nn.relu(g_4)
      g_4 = conv_concat(g_4, y)

      all_output_shape = (batch_size, 55, 46, 46, 16)
      g_5 = conv3d_transpose_tensor(g_4, 16, 3, all_output_shape, strides=strides, padding='SAME',kernel_name='kernel_5', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
      g_5 = tf.contrib.layers.batch_norm(g_5, is_training=phase_train)
      g_5 = tf.nn.relu(g_5)
      g_5 = conv_concat(g_5, y)

      all_output_shape = (batch_size, 109, 91, 91, 1)
      g_6 = conv3d_transpose_tensor(g_5, 1, 3, all_output_shape, strides=strides, padding='SAME',kernel_name='kernel_6', split_dimension_core=split_dimension_core,tt_rank=tt_rank)
  ...
  def train_all_epochs(self, config):
    ...
    for epoch in range(start_epoch, n_epochs + 1):
      ...
      mean_accuracy = self.test(
        self.data_provider.validate, batch_size_label)
